...
Pins the things the daemon and the host library rely on: the pair the firmware
computes is the pair naming.py predicts, the name persists across a reset like
the rest of the config, choosing a link by number forgets it, and `?`
advertises the command.

A relay running firmware without `!N` answers `!N?` with "unknown command";
...
def _first_board(attached, borrowed) -> str:
    ...
    if not NAME_RE.search(_talk(port, [b"HELLO\n", b"!N?\n"])):
        _restore(port)
        pytest.skip("this relay's firmware has no !N (protocol §3.7)")
    return port
...
def test_a_name_selects_the_link_naming_py_predicts(attached, borrowed):
    """The firmware and naming.py must agree byte-for-byte, or the registry
    reports a default the board does not tune to. tovez is 48/29."""
    port = _first_board(attached, borrowed)
    try:
        data = _talk(port, [b"!N Tovez\n", b"!N?\n", b"?\n"])
        assert _last_pair(data) == name_to_radio("tovez") == (48, 29)
        assert NAME_RE.findall(data)[-1] == b"tovez", "not lower-cased"
        query = data[data.rindex(b"# channel:"):]
        assert re.search(rb"power:\s*\d+\s+name:\s*tovez\s+caps:[^\r\n]*\bN\b", query), \
            f"name must follow power: and precede caps:, which lists N: {query!r}"
    finally:
        _restore(port)
...
def test_a_transient_tune_forgets_the_name_only_until_reset(attached, borrowed):
    """!CGT never touches flash, so the saved named link comes back."""
    port = _first_board(attached, borrowed)
    try:
        _talk(port, [b"!N tovez\n"])
        data = _talk(port, [b"!CGT 3 10\n", b"!N?\n"])
        assert _last_pair(data) == (3, 10)
        assert NAME_RE.findall(data)[-1] == b"-"
        reset_board(port)
        data = _talk(port, [b"HELLO\n", b"?\n", b"!N?\n"])
        assert _last_pair(data) == (48, 29)
        assert NAME_RE.findall(data)[-1] == b"tovez"
    finally:
        _restore(port)
