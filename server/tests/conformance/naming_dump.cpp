...
// decodeName() -- what `!N <name>` actually executes -- and radioToName() --
// what `!N?` runs -- execute and puts their output INTO the hashed artifact, so
// a checker verifies the inverse rather than trusting this program's own
// opinion of it. Its sha256 is the spec's D2. Pass "1" for the three-column v1
// form (D1), kept as a bisector.
//
...
int main(int argc, char **argv)
{
    ...
    for (int n = 0; n < naming::kSpace; ++n)
    {
        ...
        {
            std::fprintf(stderr, "'%s' -> (%d, %d): radioToName says no name derives it\n",
                         name, channel, group);
            return 4;
        }
        if (version == 1)
